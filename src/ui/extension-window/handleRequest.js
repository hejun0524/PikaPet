// extension-window/handleRequest.js
 ...
 * same protocol the hub serves (see doc/extensions.md). Note: widget-action messages
...
export async function handleRequest(type, payload) {
  ...
  if (type === "open-window") {
    // No `id` here — Rust derives the calling extension from this popup
    // window's own label now (see extensions::bridge::calling_extension_id),
