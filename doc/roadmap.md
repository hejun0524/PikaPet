...
|---|---|---|
| **Localization: Arena & Adventure** | The app UI is localized (en/zh/fr/es/de/ja), but the ⚔️ Adventure world (`src/ui/adventure.js` + `adventure/`) and 🥊 Arena (`src/ui/arena.js` + `arena/`) still render **English-only** — deliberately left untouched for now (huge narrative string surface in Adventure; Arena's engine is still being built) | Extract their strings through `shared/i18n.js` the same way (their view **titles** in the nav are already translated) |
