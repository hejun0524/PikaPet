...
- **🔮 Magic Station** — three sections of pet forms; forms are owned and switching between owned forms is always free and instant:
  ...
  - **🌟 Legendary Cats** — Pika, Darcy, and Noonie as EARNED forms, never sold: **Garden Cat** (Pika — visit every city and sports team), **Tuxedo Cat** (Darcy — master all 50 fight skills), **Calico Cat** (Noonie — learn every recipe). Locked cards show the quest with live progress; once earned, one click claims the form free. Cards show breeds only, like every other form. Each has its own spritesheet (`pets/garden_cat.webp`, `pets/tuxedo_cat.webp`, `pets/calico_cat.webp`).
