...
├── src/
│   ├── main.rs               # Windows, tray, menus, save-file/reset commands, add-on install/scan, music scan
...
The JS is organized as browser ES modules with **one function per file**: each window has an entry file (`hub.js`, `stats.js`, `main.js`, `setup.js`, `addon-window.js`) plus a folder of per-function modules (`hub/`, `stats/`, …); the old shared filenames (`items.js`, `school.js`, …) remain as master files that only re-export. Mutable state lives in each window's `state.js`.
...
- **UI assets are embedded in the binary at compile time** — after editing anything in `src/ui/`, rebuild (`cargo build` from the repo root). Add-on pages are the exception: they live in the user's addons directory and update by reinstalling the zip, no app restart needed.
